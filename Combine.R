install.packages(c("shiny", "shinydashboard", "plotly", "ggplot2", "dplyr", "readr", "tidyverse", "leaflet", "sf", "rnaturalearth", "rnaturalearthdata"))
library(shiny)
...
server <- function(input, output) {
  ...
  filtered_data2 <- reactive({
    clean_wide3 %>%
      filter(region_name == input$region2,
             !is.na(Year))
  })
  ...
  output$ts_plot <- renderPlot({
    ...
    ggplot(dat, aes(x = Year, y = value,
      ...
      labs(
        title = paste0("Maternal health & survival in ", input$region2),
