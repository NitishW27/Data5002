...
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      # =======================
      # DASHBOARD (overview + all 4 mini sections)
      # =======================
      tabItem(tabName = "Dashboard",
              ...
              fluidRow(
                box(
                  title = "Project Introduction", width = 12,
            ...
            interactive visualizations that reveal patterns and trends over time.",
              ...
              fluidRow(
                box(
                  title = "How to Use This Dashboard", width = 12,
                  status = "info", solidHeader = TRUE,
                  tags$ul(
                    tags$li("Use this Dashboard for a high-level overview of all visualisations."),
                    tags$li("Use the tabs in the sidebar for detailed, full-page views."),
                    tags$li("Controls below each section update only that panel.")
                  )
                )
              ),
              ...
              # ---- Dashboard Section 1: Maternal vs Life Expectancy ----
              h3("Maternal Mortality vs Life Expectancy (Dashboard View)"),
              fluidRow(
                box(
                  title = "Controls", width = 3,
                  status = "warning", solidHeader = TRUE,
                  sliderInput("year1_dash", "Select Year:",
                              min = 2010, max = 2020, value = 2015,
                              step = 5, sep = "", ticks = TRUE),
                  selectInput("regions1_dash", "Select Regions:",
                              choices = unique(clean_wide3$region_name),
                              multiple = TRUE,
                              selected = unique(clean_wide3$region_name)[1:5]),
                  checkboxInput("trend_line1_dash", "Show Trend Line", TRUE),
                  checkboxInput("label_points1_dash", "Label Regions", FALSE)
                ),
                box(
                  title = "Scatter Plot", width = 9,
                  status = "primary", solidHeader = TRUE,
                  plotOutput("scatterPlot_dash", height = 350)
                )
              ),
              
              br(), hr(), br(),
              
              # ---- Dashboard Section 2: Time Series ----
              h3("Female-Focused Mortality & Life Expectancy – Time Series (Dashboard View)"),
              fluidRow(
                box(
                  title = "Controls", width = 3,
                  status = "warning", solidHeader = TRUE,
                  selectInput("region2_dash", "Select Country/Area:",
                              choices = sort(unique(clean_wide3$region_name)),
                              selected = sort(unique(clean_wide3$region_name))[1],
                              multiple = TRUE),
                  checkboxGroupInput("vars_dash", "Indicators to show:",
                ...
                box(
                  title = "Time Series Plot", width = 9,
                  status = "primary", solidHeader = TRUE,
                  plotOutput("ts_plot_dash", height = 350)
                )
              ),
              
              br(), hr(), br(),
              
              # ---- Dashboard Section 3: Regional Comparison ----
              h3("Regional Comparison (Dashboard View)"),
              fluidRow(
                box(
                  title = "Controls", width = 3,
                  status = "warning", solidHeader = TRUE,
                  sliderInput("year3_dash", "Select Year:",
                              min = 2010, max = 2020, value = 2015,
                              step = 5, sep = "", ticks = TRUE),
                  selectInput("regions3_dash", "Compare Regions:",
                              choices = unique(clean_wide3$region_name),
                              multiple = TRUE,
                              selected = unique(clean_wide3$region_name)[1:8])
                ),
                box(
                  title = "Regional Comparison Plot", width = 9,
                  status = "primary", solidHeader = TRUE,
                  plotOutput("regionalPlot_dash", height = 350)
                )
              ),
              
              br(), hr(), br(),
              
              # ---- Dashboard Section 4: Choropleth Map ----
              h3("Global Health Indicators – Choropleth Map (Dashboard View)"),
              fluidRow(
                box(
                  title = "Map Controls", width = 3,
                  status = "warning", solidHeader = TRUE,
                  sliderInput("year4_dash", "Select Year:",
                              min = 2010, max = 2024, value = 2015,
                              step = 5, sep = ""),
                  selectInput("indicator_dash", "Select Indicator to Map:",
                              choices = c(
                ...
                box(
                  title = "Interactive World Map", width = 9,
                  status = "primary", solidHeader = TRUE,
                  leafletOutput("map_dash", height = 400)
                )
              )
      ),
...
server <- function(input, output, session) {
  ...
  # =======================
  # VIZ 1: Maternal vs Life Expectancy
  # =======================
  ...
  # Dashboard version of VIZ 1
  filtered_data1_dash <- reactive({
    req(input$year1_dash, input$regions1_dash)
    clean_wide3 %>%
      filter(Year == input$year1_dash,
             region_name %in% input$regions1_dash)
  })
  
  output$scatterPlot_dash <- renderPlot({
    data <- filtered_data1_dash()
    ...
    if (input$trend_line1_dash) {
      p <- p + geom_smooth(method = "lm", se = FALSE, color = "darkred", linetype = "dashed")
    }
    
    if (input$label_points1_dash) {
      p <- p + geom_text(aes(label = region_name), vjust = -0.5, hjust = 0.5, size = 3)
    }
  ...
  # Dashboard version of VIZ 2
  filtered_data2_dash <- reactive({
    req(input$region2_dash)
    clean_wide3 %>%
      filter(region_name %in% input$region2_dash,
             !is.na(Year))
  })
  
  ts_long_dash <- reactive({
    req(input$vars_dash)
    
    dat <- filtered_data2_dash()
    all_names <- names(dat)
    indicator_cols <- character(0)
    
    if ("mmr" %in% input$vars_dash) {
      idx <- grep("maternal.*mortality", all_names, ignore.case = TRUE)
      indicator_cols <- c(indicator_cols, all_names[idx])
    }
    if ("u5mr" %in% input$vars_dash) {
      idx <- grep("under.*five.*mortality", all_names, ignore.case = TRUE)
      indicator_cols <- c(indicator_cols, all_names[idx])
    }
    if ("fle" %in% input$vars_dash) {
      idx <- grep("life_expectancy.*females", all_names, ignore.case = TRUE)
      indicator_cols <- c(indicator_cols, all_names[idx])
    }
  ...
  output$ts_plot_dash <- renderPlot({
    ...
    yr_range <- range(dat$Year, na.rm = TRUE)
    region_label <- paste(input$region2_dash, collapse = ", ")
  ...
  # Dashboard version of VIZ 3
  filtered_data3_dash <- reactive({
    req(input$year3_dash, input$regions3_dash)
    clean_wide3 %>%
      filter(Year == input$year3_dash,
             region_name %in% input$regions3_dash)
  })
  
  output$regionalPlot_dash <- renderPlot({
    data <- filtered_data3_dash()
    ...
    ggplot(data, aes(x = reorder(region_name, Life_expectancy_at_birth_for_both_sexes_years), 
      ...
      coord_flip() +
      labs(x = "Region", y = "Life Expectancy at Birth (years)",
           title = paste("Life Expectancy by Region -", input$year3_dash)) +
      theme_minimal() +
      theme(legend.position = "none")
  })
  
  # =======================
  # VIZ 4: Choropleth Map
  # =======================
  ...
  # Dashboard version of VIZ 4
  map_data_reactive_dash <- reactive({
    req(input$year4_dash)
    
    world %>%
      left_join(
        map_data %>% filter(Year == input$year4_dash),
        by = "region_name"
      )
  })
  
  observeEvent(input$indicator_dash, {
    if (input$indicator_dash == "Maternal_mortality_ratio_deaths_per_100_000_population") {
      updateSliderInput(
        session, "year4_dash",
        min = 2010,
        max = 2020,
  ...
  output$map_dash <- renderLeaflet({
    dat <- map_data_reactive_dash()
    var <- input$indicator_dash
    
    labels <- sprintf(
      "<strong>%s</strong><br/>%s: %s<br/>Year: %s",
      dat$region_name,
      names(which(choices == input$indicator_dash)),
      ifelse(is.na(dat[[var]]), "No data", round(dat[[var]], 2)),
      input$year4_dash
    ) %>% lapply(htmltools::HTML)
    ...
    pal <- colorNumeric(
      ...
      domain = dat[[var]],
      na.color = "#f0f0f0",
      reverse = input$indicator_dash %in% c("Life_expectancy_at_birth_for_females_years", 
                                            "Life_expectancy_at_birth_for_both_sexes_years")
    )
    ...
    leaflet(dat) %>%
      ...
      addLegend(
        ...
        values = dat[[var]],
        position = "bottomright",
        title = names(which(choices == input$indicator_dash)),
        opacity = 0.85,
        na.label = "No Data"
      )
  })
